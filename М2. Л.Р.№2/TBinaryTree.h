...
struct TTreeNotExistException
	: public std::exception
{
	TTreeNotExistException(char const* const message) throw()
		: std::exception(message)
	{
	}
};
...
public:
	...
	{
		if (Root == nullptr)
			throw TTreeNotExistException("Error! Binary tree not exist");
		...
		throw TNotFoundException("Error! Element not found");
	}
